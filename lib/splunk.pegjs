  ...
  */
  function replaceAppliationName2Id(names) {
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      let appIds = [];
      // 先精确查找
      appIds = ALL_APPLICATIONS_LIST.filter(
        app =>
          app.name &&
          app.name.toLocaleUpperCase() ===
            appName.trim().toLocaleUpperCase()
      ).map(app => app.id);
  ...
  function replaceAppliationName2IdWithLike(name) {
    ...
    const appName = name ? name.trim() : '';
    // 如果查询的应用名字都是通配符，则直接返回-1
    if (appName.split('').every(ch => ch === '%' || ch === '_')) {
      return [-1]
    }
    let result = [];
    let appIds = [];
      ...
      app => {
        if(appName.indexOf('%') === -1 && appName.indexOf('_') === -1) {
          return app.name && app.name
            .toLocaleUpperCase()
            .indexOf(appName.toLocaleUpperCase()) > -1;
        }
        ...
        const lastChar = appName.charAt(appName.length);
  ...
  const { applications, json = false, hasAgingTime = true } = options;
...
SplExpression "SplExpression"
  = sourceAndsearch:SourceAndSearchExpression tailArr:(_? divider _? TailCommand)* __? {
    ...
    if(tailMap.GENTIMES) {
      ...
      if(hasAgingTime) {
        timeSql += '(';
        timeSql += ` start_time >= toDateTime64(:param_aging_start_time, 9, 'UTC') `;
        timeSql += ` and start_time < toDateTime64(:param_end_time, 9, 'UTC') `;
        timeSql += ` and end_time >= toDateTime64(:param_start_time, 9, 'UTC') `;
        timeSql += ')';
      paramsValuesObj['param_aging_start_time'] = agingStartTime;
      } else {
      timeSql += '(`' + time_field + '`'  + `>= toDateTime64(:param_start_time, 9, 'UTC') AND ` + '`' +  time_field + '`' + ` <= toDateTime64(:param_end_time, 9, 'UTC'))`;
...
LikeCondition "LikeCondition"
 = fieldObj:FieldWithType __ LIKE __ value:LikeValue {
    ...
    const { paramKey, paramName } = getLocation(fieldName, fieldType);
    if(fieldName === APPLICATION_NAME_KEY) {
      const newValues = replaceAppliationName2IdWithLike(value);
      updateParamsValues(paramKey, newValues);
      return '`' + APPLICATION_ID_KEY + '`' + ' IN ' + '(' + paramKey + ')';
    }
