...
SplExpression "SplExpression"
  = sourceAndsearch:SourceAndSearchExpression tailArr:(_? divider _? TailCommand)* __? {
    ...
    if(tailMap.GENTIMES) {
      ...
      timeSql += '(';
      timeSql += ` start_time >= toDateTime64(${agingStartTime}, 9, 'UTC') `;
      timeSql += ` and start_time < toDateTime64(${endTime}, 9, 'UTC') `;
      timeSql += ` and end_time >= toDateTime64(${startTime}, 9, 'UTC') `;
      timeSql += ')';
