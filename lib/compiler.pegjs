{
  ...
  */
  ...
  // 可以有纳秒（9）和毫秒（3）2个选项
  const {  json = false, hasAgingTime = true, timePrecision = 9, includeStartTime = true, includeEndTime = true  } = options;
}
...
SplExpression "SplExpression"
  = sourceAndsearch:SourceAndSearchExpression tailArr:(_? divider _? TailCommand)* __? {
    ...
    if(tailMap.GENTIMES) {
      ...
      // 处理时间包含关系
      const startTimeOperator = includeStartTime ? '>=' : '>';
      const endTimeOperator = includeEndTime ? '<=' : '<';
      ...
      if(hasAgingTime) {
        ...
        timeSql += '(`' + time_field + '`'  + `${startTimeOperator} toDateTime64(:param_start_time, ${timePrecision}, 'UTC') AND ` + '`' +  time_field + '`' + ` ${endTimeOperator} toDateTime64(:param_end_time, ${timePrecision}, 'UTC'))`;
      }
...
BasicCondition "BasicCondition"
  = fieldObj:FieldWithType __ op:(equal / notEqual / gte / gt / lte / lt) __ value:Value {
    ...
    if(fieldType === 'Array') {
      ...
      const splitValue = value.split(',').filter(item => item);
      if(splitValue.length >= 2) {
        ...
        return ` hasAll(${fieldName}, [${paramNameArr.join(',')} ])=1`;
      }
    }
